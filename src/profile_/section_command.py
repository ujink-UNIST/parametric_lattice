...
def build_section_commands_(
    ...
    for edge, beam_type_id, ratio in zip(
        ...
        if sec_id is None:
            ...
            # For this repo's pipelines/tests, the edge already carries the
            # normalized diameter ratio (e.g. 1.0), so we keep it as-is.
...
def _get_edge_radius_ratio(
    ...
    if radius_ratio is None:
        raise KeyError(
            f"Beam type {beam_type_id} is missing 'radius_ratio'"
        )
